...
### 4.4 路径 / trusted project 差异

通常表现为：

- 主仓库路径里 hooks 正常，复制出来的新路径却没有反应
- repo-local 文件都在，但 `.harness/state/current/*` 没有刷新
- 你在 `/tmp` 或新的副本目录里复现不出主仓库的行为

判断方式：

- 先确认主仓库路径是否已经验证通过
- 再确认新路径是否被宿主视为 trusted project
- 如果主仓库正常、副本失效，优先记为“路径 / 信任上下文差异”，不要直接断言模板坏了
